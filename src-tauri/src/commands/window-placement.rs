...
pub const CONTENT_WINDOW_GAP: i32 = 8;
...
/// Calculates a content window position centered below the expanded overlay.
pub fn calculate_content_window_position(
    overlay_position: PhysicalPosition<i32>,
    overlay_size: PhysicalSize<u32>,
    content_window_size: PhysicalSize<u32>,
    work_area: Option<PhysicalRect<i32, u32>>,
) -> PhysicalPosition<i32> {
    let overlay_center_x = i64::from(overlay_position.x) + i64::from(overlay_size.width) / 2;
    let centered_x = overlay_center_x - i64::from(content_window_size.width) / 2;
    let below_y = i64::from(overlay_position.y)
        + i64::from(overlay_size.height)
        + i64::from(CONTENT_WINDOW_GAP);

    let (x, y) = match work_area {
        Some(area) => {
            let area_x = i64::from(area.position.x);
            let area_y = i64::from(area.position.y);
            let max_x = area_x + i64::from(area.size.width) - i64::from(content_window_size.width);
            let max_y =
                area_y + i64::from(area.size.height) - i64::from(content_window_size.height);
            (
...
mod tests {
    ...
    #[test]
    fn centers_content_window_below_the_overlay() {
    ...
    #[test]
    fn keeps_content_window_inside_a_work_area() {
    ...
    #[test]
    fn anchors_an_oversized_content_window_to_the_work_area_origin() {
        let work_area = PhysicalRect {
